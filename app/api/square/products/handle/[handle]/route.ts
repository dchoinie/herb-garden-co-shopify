...
export async function GET(
  ...
  try {
    ...
    // Search for items by name (since Square doesn't have handles, we'll search by name)
    const searchRequest = {
      objectTypes: ["ITEM"],
      query: {
        textQuery: {
          keywords: [handle.replace(/-/g, " ")],
        },
      },
      limit: 10,
    };
    ...
    const item =
      response.result.objects.find((obj: any) => {
        const itemName = obj.itemData?.name
          ?.toLowerCase()
          .replace(/[^a-z0-9]+/g, "-");
        return itemName === handle;
    ...
    console.error("Error fetching Square product by handle:", error);
    return NextResponse.json(
